#include "NDelayFunc.h"
#include "ArduinoJson.h"
#include <stdlib.h>
#include "WiFiEsp.h"
#include "SoftwareSerial.h"
#include <math.h>
#include <OneWire.h> 
#include <DallasTemperature.h>

#define SensorPin A0            //pH meter Analog output to Arduino Analog Input 0
#define Offset 0.00            //deviation compensate
#define LED 13
#define ArrayLenth  20    //times of collection // default 40
#define ONE_WIRE_BUS 9
int pHArray[ArrayLenth];  //Store the average value of the sensor feedback
int pHArrayIndex = 0;
static float pHValue, voltage;
float temp;

const int pumpPin1 = 4;
const int pumpPin2 = 5;
unsigned int on_off = 0;

SoftwareSerial Serial1(2, 3); // TX, RX

char ssid[] = "HYOSUNG-2G"; // 공유기 WiFi 이름
char pass[] = "hshs7001"; // 공유기 비밀번호
char server[] = "192.168.0.100"; // 여러분의 장고 서버IP주소

WiFiEspClient client;

OneWire oneWire(ONE_WIRE_BUS); // Setup a oneWire instance to communicate with any OneWire devices 
DallasTemperature sensors(&oneWire); // Pass our oneWire reference to Dallas Temperature. 


void getherringSensors(){
    //pH part
    pHArray[pHArrayIndex++] = analogRead(SensorPin);
    if (pHArrayIndex == ArrayLenth)pHArrayIndex = 0;
    voltage = avergearray(pHArray, ArrayLenth) * 5.0 / 1024;
    pHValue = 3.5 * voltage + Offset;

    //temperature part 
    sensors.requestTemperatures(); // Send the command to get temperature readings
    temp=sensors.getTempCByIndex(0); // You can have more than one DS18B20 on the same bus. 
                                     //0 refers to the first IC on the wire
}

void printSensors(){
    Serial.print("Voltage:");
    Serial.println(voltage, 2);
    Serial.print("pH value: ");
    Serial.println(pHValue, 2);
    Serial.print("Temperature: "); 
    Serial.println(temp);
    Serial.println("DONE");
    
    digitalWrite(LED, digitalRead(LED) ^ 1);
}


void getHeaderValue(char* src, char* dst, char* start) {
  char* pStart = strstr(src, start);
  int cnt = 0;
  if (pStart != NULL) {
    for (int i = strlen(start); pStart + i != strstr(pStart, "\r\n"); i++) {
      dst[cnt++] = pStart[i];
    }
    dst[cnt] = '\0';
  }
}


void httpRequestPost() {
  client.stop();

  if (client.connect(server, 8000)) {
    /*
       2. POST 요청
    */
    String body = "";
    DynamicJsonDocument doc(128);
    doc["T"] = temp;
    doc["pH"] = pHValue;
    doc["type"]= "Pumps";
    doc["action_result"] = on_off == 0 ? "on" : "off";
    doc["message"] = "success";
    serializeJson(doc, body);

    // Request Header 정보와 직렬화된 JSON 객체를 서버로 전달
    client.print("POST /api/arduino/ HTTP/1.1\r\n");
    client.print("Host: ");
    client.print(server);
    client.print(":8000\r\n");
    client.print("Content-Type: application/json\r\n");
    client.print("Content-Length: ");
    client.print(body.length());
    client.print("\r\n\r\n");
    client.print(body);
    client.print("\r\n");
    Serial.println("Transmission complited !!!!!!!");

    // 응답 내용을 저장할 메모리 할당
    char* buf = (char*)malloc(sizeof(char) * 512); // 512Byte 크기 할당 (동적배열할당 / C언어 내용)
    int index = 0;

    while (client.connected()) {
      if (client.available()) { // 서버로부터 전달받은 응답 중 아두이노가 처리하지 않은 내용이 존재하는지 여부
        char c = client.read();
        buf[index++] = c;
      }
      else {
        client.stop();
      }
    }

    // 응답 내용 중 서버로부터 전달받은 데이터의 길이 값을 가져온다.
    char* dataLength = (char*)malloc(sizeof(char) * 50); // 50Byte 메모리 할당
    getHeaderValue(buf, dataLength, "Content-Length: ");
    int length = atoi(dataLength); // 문자열을 정수로 변환

    // JSON 문자열을 응답 문자열로부터 끄집어 내기
    if (length > 0) {
      for (int i = 0; i < strlen(dataLength); i++) {
        dataLength[i] = '\0';
      }

      int cnt = 0;
      char* body = strstr(buf, "\r\n\r\n");
      int newLineLength = strlen("\r\n\r\n");
      for (int i = newLineLength; i < length + newLineLength; i++) {
        dataLength[cnt++] = body[i];
      }
      dataLength[cnt] = '\0';
      Serial.print("JSON 문자열: ");
      Serial.println(dataLength);

      // JSON 문자열을 JSON 객체로 변경
      StaticJsonDocument<50> doc;
      deserializeJson(doc, dataLength);
      const char* message = doc["message"];
      Serial.println(message);
    }
    free(dataLength);

    // 응답 내용을 출력
    for (int i = 0; i < index; i++) {
      Serial.print(buf[i]);
    }
    free(buf); // 할당 받았던 메모리를 해제
  }
}

NDelayFunc nDelayHttpRequestPost(30000, httpRequestPost);
NDelayFunc nDelayGetherringSensors(20,getherringSensors);
NDelayFunc nDelayPrintSensors(1111,printSensors);

void setup() {
  pinMode(LED, OUTPUT);

  
  // ESP 모듈 초기화
  Serial1.begin(9600);
  Serial.begin(9600);
  WiFi.init(&Serial1);
  Serial.print("연결을 시도 중 입니다. WPA SSID: ");
  Serial.println(ssid);

  // WiFi 네트워크 연결
  WiFi.begin(ssid, pass);
  Serial.println("ESP Module Check !");
  Serial.println("연결 되었습니다.");

   // SEN0161 
  
  Serial.println("SEN0161 Check!");    //Test the serial monitor

  //JT-180A 수중모터
  
  Serial.println("JT-180A Check!");
  pinMode(pumpPin1, OUTPUT);
  pinMode(pumpPin2, OUTPUT);

  //DS18B20 
  Serial.println("DS18B20 Check !");  
  sensors.begin(); // Start up the library

}

void loop() {

  nDelayHttpRequestPost.run();
  
  nDelayGetherringSensors.run();

  nDelayPrintSensors.run();

  if(pHValue>6 && pHValue<8){
    on_off=1;             
    digitalWrite(pumpPin1,on_off);
    digitalWrite(pumpPin2,on_off);
  }
  else {
    on_off=0;
    digitalWrite(pumpPin1,on_off); // LOW: 작동 , HIGH: 비작동
    digitalWrite(pumpPin2,on_off);
  }

  
}

double avergearray(int* arr, int number) {
  int i;
  int max, min;
  double avg;
  long amount = 0;
  if (number <= 0) {
    Serial.println("Error number for the array to avraging!/n");
    return 0;
  }
  if (number < 5) { //less than 5, calculated directly statistics
    for (i = 0; i < number; i++) {
      amount += arr[i];
    }
    avg = amount / number;
    return avg;
  } else {
    if (arr[0] < arr[1]) {
      min = arr[0]; max = arr[1];
    }
    else {
      min = arr[1]; max = arr[0];
    }
    for (i = 2; i < number; i++) {
      if (arr[i] < min) {
        amount += min;      //arr<min
        min = arr[i];
      } else {
        if (arr[i] > max) {
          amount += max;  //arr>max
          max = arr[i];
        } else {
          amount += arr[i]; //min<=arr<=max
        }
      }//if
    }//for
    avg = (double)amount / (number - 2);
  }//if
  return avg;
}
